...
namespace Ecommerce.Infrastructure.Persistence.Seeders
{
    ...
    {
        private readonly string _prefix = "EMAS";
        public override IEnumerable<EmailStatus> GetSeedData()
        {
            ...
            {
                yield return new EmailStatus
                {
                    Id = GenerateDeterministicGuid(status),
                    Name = status,
                    Flag = GenerateFlag(status),
                    FriendlyCode = $"{_prefix}{GenerateFriendlyCode(className, status)}",
                    Description = $"Define o estado de {status.ToLower()} no ciclo de vida do processamento de e-mails.",
